...
================================================================================
Master 4-Tier E2E Test Suite Runner
================================================================================
Executes Tiers 1 through 4 in isolated subprocesses, aggregates results across
all 28 features (Requirements R1 through R6), formats terminal output,
and exports comprehensive JSON results.
================================================================================
"""
...
def run_all_tiers():
    ...
    tier_scripts = [
        ("Tier 1 - Feature Coverage", BASE_DIR / "tests" / "e2e" / "test_tier1_feature_coverage.py"),
        ("Tier 2 - Boundary & Corner Cases", BASE_DIR / "tests" / "e2e" / "test_tier2_boundary_corner.py"),
        ("Tier 3 - Cross-Feature Pairwise", BASE_DIR / "tests" / "e2e" / "test_tier3_cross_feature.py"),
        ("Tier 4 - Real-World Scenarios", BASE_DIR / "tests" / "e2e" / "test_tier4_real_world.py"),
    ]

    print("\n" + "=" * 80)
    print("  J.A.R.V.I.S. Autonomous Operations & Mobile Companion Ecosystem")
    print("  Master 4-Tier Opaque-Box E2E Test Suite (Requirements R1-R6)")
    print("=" * 80 + "\n")
    ...
    for tier_name, script_path in tier_scripts:
        ...
        print(f"[Executing] {tier_name} ({script_path.name})...")
        ...
        try:
            with open(temp_log, "w", encoding="utf-8", errors="replace") as log_f:
                proc = subprocess.Popen(
                    [sys.executable, "-m", "unittest", str(script_path)],
                    cwd=str(BASE_DIR),
                    stdout=log_f,
                    stderr=log_f,
                )
        ...
        status = "PASS" if returncode == 0 and failures == 0 and errors == 0 and tests_run > 0 else "FAIL"
