"""Automated Companion Unit Test for dynamic skill: server_status_ping"""
...
class TestDynamicSkill_server_status_ping(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.mod = importlib.import_module("skills.server_status_ping")

    def test_manifest_structure(self):
        self.assertTrue(hasattr(self.mod, "MANIFEST"), "Module must expose MANIFEST")
        manifest = self.mod.MANIFEST
        self.assertIsInstance(manifest, dict)
        self.assertEqual(manifest.get("name"), "server_status_ping")
    ...
    def test_run_with_valid_parameters(self):
        params = {"ports": [8770, 8765], "host": "test_sample"}
