...
def _is_destructive(tool: str, parameters: dict, goal: str = "") -> bool:
    """Safety net: block catastrophic whole-drive / system-wide operations."""
    text = (json.dumps(parameters or {}) + " " + (goal or "")).lower()
    catastrophic = [
        "format c", "format d", "format e",
        "del /f /s", "del /s /q", "rmdir /s", "rd /s",
        "remove-item -recurse", "remove-item -path c:\\", "move-item c:\\",
        "robocopy c:\\", "/mir", "shutil.rmtree", "rm -rf",
        "diskpart", "clean all",
    ]
    if any(p in text for p in catastrophic):
        return True
    # Generic "move/clear/wipe an ENTIRE drive or all files" intent.
    bulk = any(w in text for w in ("all files", "everything", "entire drive", "whole drive", "clear drive"))
    drive = any(w in text for w in ("drive c", "drive d", "drive e", "c:\\", "d:\\", "e:\\"))
    if bulk and drive:
        return True
    return False
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool in ("cmd_control", "file_controller") and _is_destructive(tool, parameters):
        msg = ("BLOCKED for safety: that is a destructive whole-drive or system "
               "operation. I will not do that automatically — please confirm and do "
               "it manually if you really intend it, Sir.")
        if speak:
            try:
                speak("I've blocked that — it looked like a destructive drive operation. Please do it manually if you really intend it, sir.")
            except Exception:
                pass
        return msg
